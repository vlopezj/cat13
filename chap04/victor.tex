        ...
        \item
          ...
          \begin{itemize}
            \item % PF = PG
              ...
              Now, if there is a functor $Z : D -> X$, then there is
              a unique functor $\bar{Z} : \nicefrac{D}{~} → X$, such that

              $\bar{Z} \comp P = Z$. 

              $\bar{Z} = Z \comp I$, where $I$ is a mapping that chooses
              a representative from the equivalence class.

              % TODO: Well defined
              % TODO: Unique
                If it's equal over classes, then it's unique by definition.
